...
class StarterBot(object):
    ...
    def get_users(self):
        ...
        for user in users:
            ...
            "'name of your charity' $20.00_ \n*hello*, *hi*, *hey*, *sup* : to say hi to the bot!", "not sure what you mean. message starterbot by typing 'commands' " + 
    ...
    def handle_command(self, command, channel, user):
        ...
        #The rest of the commands
            ...
            self._response[user] = "Not sure what you mean. Message starterbot by typing 'commands' to find all the available commands"
            self._attempting_to_donate[user] = False
            self._first_donate_message_occured[user] = False   
...
if __name__ == "__main__":
    ...
    if starter_bot.get_slack_client().rtm_connect():
        starter_bot.get_users()
        print("StarterBot connected and running!")
        while True:
            output_list = starter_bot.get_slack_client().rtm_read()
            channel, user = starter_bot.get_channel_user_from_rtm(output_list)
            starter_bot.set_channel(channel)
            command = None
            if channel != None and user != None:
                ...
                if channel.startswith('D'):
                    command = starter_bot.sending_direct_messages(output_list, user)
                else: 
                    command = starter_bot.sending_message_in_channels(output_list)
            if command and channel:
                starter_bot.handle_command(command, channel, user)
